...
class AMixFFN(nn.Module):
    ...
    def forward(self, x, H, W):
        shortcut = self.fc1(x)
        x = self.dwconv(shortcut, H, W) + shortcut
...
class AdvancedTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.mlp = AMixFFN(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class AdvancedTransformerBlock_PGF(nn.Module):
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        if self.shift_size > 0:
            ...
            x_up = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        ...
        # reverse cyclic shift
        if self.shift_size > 0:
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2, 3))
        else:
            x = shifted_x
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            AdvancedTransformerBlock(dim=dim, input_resolution=input_resolution,
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        for index, blk in enumerate(self.blocks):
            if index == 0:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, x, attn_mask, skip=skip, x_up=x_up)
                else:
                    x = blk(x, attn_mask, skip=skip, x_up=x_up)
            else:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, x)
                else:
...
class ATFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        super().__init__()
        print(
            "ATFormer expand initial----depths:{};depths_decoder:{};drop_path_rate:{};num_classes:{}".format(
                depths,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand(input_resolution=(img_size // patch_size, img_size // patch_size),
